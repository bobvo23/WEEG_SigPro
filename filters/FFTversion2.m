...
for k=1:1
    %snr = 20 * log10((ymax/meanY)^2);
    snrtemp = 20 * log10((interestY(c(k))/meanY));
    strmax = ['Max= ',num2str(interestF(c(k))),'Hz,',' SNR= ',num2str(snrtemp),'db'];
    %strmax = ['Max= ',num2str(xmax)];
...
for k2=1:1
    %snr = 20 * log10((ymax/meanY)^2);
    snrtemp2 = 20 * log10((interestY2(c2(k2))/meanY2));
 
    
    strmax2 = ['Max= ',num2str(interestF2(c2(k2))),'Hz, ',' SNR= ',num2str(snrtemp2),'dB'];
    %strmax = ['Max= ',num2str(xmax)];
...
spectro{indexdatasaved}={f2,y2,t1,c2,snrtemp2};
SNRmax(indexdatasaved) =  snrtemp2;
